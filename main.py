...
#
#         # Обрабатываем подстановки в итоговом словаре
...
#             return self.parse_dict(value[1:-1])
#         else:
...
class ConfigParser:
    ...
    def parse_value(self, value):
        ...
        if value.isdigit():  # Числа
            ...
            return self.parse_dict(value[1:-1])  # Здесь добавим обработку вложенных словарей
        else:
            raise ValueError(f"Invalid value: {value}")
    ...
    def parse_dict(self, body):
        """Парсит тело словаря в формате 'ключ => значение'."""
        result = {}
        items = self.tokenize_dict_items(body)  # Используем новый метод для правильного разделения элементов
        for item in items:
            if match := re.match(r'([a-z]+)\s*=>\s*(.+)', item.strip()):
                key, value = match.groups()
                result[key] = self.parse_value(value.strip())
    ...
    def tokenize_dict_items(self, body):
        ...
        # Для корректного разделения вложенных словарей используем регулярное выражение,
        # которое работает с вложенными конструкциями
        depth = 0
        current_item = []
        items = []
